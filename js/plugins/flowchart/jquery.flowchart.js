    ...
    $.widget("flowchart.flowchart", {
        ...
        _initEvents: function () {
            ...
            this.objs.layers.operators.on('click', '.flowchart-operator-connector', function () {
                var $this = $(this);
                if (self.options.canUserEditLinks && self.options.expectsInput === false ) {
            ...
            this.objs.layers.operators.on('click', '.flowchart-operator', function (e) {
                ...
                if (self.options.expectsInput){
                    $connector = ($this.find(".flowchart-operator-inputs .flowchart-operator-connector"));
                    $sub = $this.find('.flowchart-operator-connector-set');
                    self._connectorClicked($this.data('operator_id'), $connector.data('connector'), $connector.data('sub_connector'), $sub.data('connector_type'));
                    self.options.expectsInput = false;
                }
        ...
        setLinkData: function (linkId,data) {

            if (this.data.links.hasOwnProperty(linkId)) {
                this.data.links[linkId] = data;
            }
        ...
        addLink: function (linkData) {
            ...
            if (linkData.fromConnector === "output_0")
                linkId = linkData.fromOperator;
            else if (linkData.fromConnector === "success_0")
                linkId = linkData.fromOperator+"_s0";
            else if (linkData.fromConnector === "failure_0")
                linkId = linkData.fromOperator+"_f0";
            else
                linkId = linkData.fromConnector;
        ...
        _drawLink: function (linkId) {
            ...
            var toConnector = toOperator.internal.els.connectorArrows[toConnectorId][toSubConnector];

            linkData.internal.els.fromSmallConnector = fromSmallConnector;
            linkData.internal.els.toSmallConnector = toSmallConnector;
            linkData.internal.els.toConnector = toConnector;
            ...
            var shape_id = Flowcomposer.generateID();
            ...
            shape_path.setAttribute("id", shape_id);
            ...
            var text1 = document.createElementNS("http://www.w3.org/2000/svg", "image");
            text1.setAttribute("xlink:href", "https://localhost/clever-app/saas/images/settings.svg");
            text1.setAttribute("x", "500");
            text1.setAttribute("y", "70");
            text1.setAttribute("width", "70");
            text1.setAttribute("height", "70");
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            var operatorType = infos.type;

            var $operator = $('<div class="flowchart-operator" id="'+infos.id+'"></div>');
            $operator.addClass(infos.class);

            var $operator_title = $('<div class="flowchart-operator-title"></div>');
            $operator_title.html(infos.title+"<i class='titleicon "+infos.icon+"'></i>");
            ...
            var $operator_content = "";
            $('<div class="flowchart-operator-content"><span id="'+infos.id+'_text_message"> </span></div>');
              //  <div class="visual_img_preview" style="background-image: url('https://media1.giphy.com/media/uFtywzELtkFzi/200.gif?cid=e1bb72ff5b173fdf686f415541a1db00');"></div>

            var self = this;

            let analyticsColumnsLimited = ('<div class="flowchart-stats-footer-column">Executed<div class="deliveries-stats flowchart-stats-footer-column-value"></div></div>');
            let analyticsColumnsFlow = ('<div class="flowchart-stats-footer-column">Redirected<div class="deliveries-stats flowchart-stats-footer-column-value"></div></div>');


            let analyticsColumns = ('<div class="flowchart-stats-footer-column">Delivered<div class="deliveries-stats flowchart-stats-footer-column-value"></div></div>' +
                                  '<div class="flowchart-stats-footer-column">Opened<div class="opens-stats flowchart-stats-footer-column-value"></div></div>');

            let analyticsColumnsExtended = analyticsColumns +('<div class="flowchart-stats-footer-column">Clicked<div class="clicks-stats flowchart-stats-footer-column-value"></div></div>');


            $cardAnalyticsLimitedA =$('<div class="'+infos.id+'-stats flowchart-stats-footer-container">' +
                '<div class="flowchart-stats-footer flowchart-stats-footer-percentage">' +
                analyticsColumnsLimited +
                '</div><div class="flowchart-stats-footer flowchart-stats-footer-number">' +
                analyticsColumnsLimited +
                '</div>' +
                '</div>');

            $cardAnalyticsLimitedFlow =$('<div class="'+infos.id+'-stats flowchart-stats-footer-container">' +
                '<div class="flowchart-stats-footer flowchart-stats-footer-percentage">' +
                analyticsColumnsFlow +
                '</div><div class="flowchart-stats-footer flowchart-stats-footer-number">' +
                analyticsColumnsFlow +
                '</div>' +
                '</div>');

            $cardAnalyticsLimited =$('<div class="'+infos.id+'-stats flowchart-stats-footer-container">' +
                '<div class="flowchart-stats-footer flowchart-stats-footer-percentage">' +
                analyticsColumns +
                '</div><div class="flowchart-stats-footer flowchart-stats-footer-number">' +
                analyticsColumns +
            ...
            $cardAnalyticsExtended = $('<div class="'+infos.id+'-stats flowchart-stats-footer-container">' +
                '<div class="flowchart-stats-footer flowchart-stats-footer-percentage">' +
                analyticsColumnsExtended +
                '</div>' +
                '<div class="flowchart-stats-footer flowchart-stats-footer-number">' +
                analyticsColumnsExtended +
                '</div>' +
                '</div>');

            let $cardActionsExtended =$(
                '<div class="flowchart-operator-footer">' +
                '<div class="card-make-first" style="left: 10px" ><a href="#" data-action="make-card-first" data-card-id="'+infos.id+'" data-card-type="'+infos.type+'"> <img src="'+Flowcomposer.secondaryCardIcon+'" width="10px" class="first-inactive"/></a></div>' +
                '<div class="card-edit-settings" data-card-id="'+infos.id+'" data-card-type="'+infos.type+'"><a href="#"> <i class="fa icon-cog" aria-hidden="true"></i></a></div>' +
                '<div class="card-duplicate" data-card-id="'+infos.id+'" data-card-type="'+infos.type+'"><a href="#"> <i class="fa icon-copy" aria-hidden="true"></i></a></div>' +
                '<div class="card-delete" data-action="delete-card" data-card-id="'+infos.id+'" data-card-type="'+infos.type+'"><a href="#"> <i class="fa icon-cross" data-id="'+infos.id+'"></i></a></div>' +
                '</div>');

            let $cardActionsLimited =$(
                '<div class="flowchart-operator-footer">' +
                '<div class="card-edit-settings" data-card-id="'+infos.id+'" data-card-type="'+infos.type+'"><a href="#"> <i class="fa icon-cog" aria-hidden="true"></i></a></div>' +
                '<div class="card-duplicate" data-card-id="'+infos.id+'" data-card-type="'+infos.type+'"><a href="#"> <i class="fa icon-copy" aria-hidden="true"></i></a></div>' +
                '<div class="card-delete" data-action="delete-card" data-card-id="'+infos.id+'"><a href="#"> <i class="fa icon-cross" data-id="'+infos.id+'"></i></a></div>' +
                '</div>');

            // Backward compatibility for category classes
            if (typeof infos.category === "undefined" || (!infos.category)) {
                let category = "default";
                switch (operatorType) {
                    //message types up until wowing
                    case "text":
                    case "image":
                    case "video":
                    case "list":
                ...
                $operator.addClass(category);

            }
            else
                $operator.addClass(infos.category);
            ...
            switch (operatorType) {
                ...
                case "text":
                    $cardActions = $cardActionsExtended;
                    $cardAnalytics = $cardAnalyticsExtended;
                    $operator_content = $('<div class="flowchart-operator-content">' +
                        '<div class="text-card-incard-preview"><span id="'+infos.id+'_message" class="card-text-preview"> </span></div></div>');
                ...
                case "list":
                    ...
                    $cardAnalytics = $cardAnalyticsExtended;
                    $operator_content = $('<div class="flowchart-operator-content">' +
                        '<span id="'+infos.id+'_message" class="card-text-preview"> </span></div>');
                ...
                case "image":
                    ...
                    $cardAnalytics = $cardAnalyticsExtended;
                    $operator_content = $('<div class="flowchart-operator-content"' +
                        '><div id="'+infos.id+'_message" class="" ></div></div>');
                ...
                case "video":
                    ...
                    $cardAnalytics = $cardAnalyticsExtended;
                    $operator_content = $(
                        '<div class="flowchart-operator-content">' +
                        '<div id="'+infos.id+'_message" class="card-video-preview">' +
                        '</div></div>');
                    break;

                case "audio":
                    $cardActions = $cardActionsExtended;
                    $cardAnalytics = $cardAnalyticsLimited;
                    $operator_content = $(
                        '<div class="flowchart-operator-content">' +
                        '<div id="'+infos.id+'_message" class="card-audio-preview">' +
                        '</div></div>');
                    break;
                ...
                case "active-campaign":
                    $cardActions = $cardActionsExtended;
                    $cardAnalytics = $cardAnalyticsLimitedA;
                    $operator_content = $('<div class="flowchart-operator-content">' +
                        '<span id="'+infos.id+'_message" class="card-text-preview"> </span></div>');
                    break;
                ...
                case "demio":
                    ...
                    $cardAnalytics = $cardAnalyticsLimitedA;
                    $operator_content = $('<div class="flowchart-operator-content">' +
                        '<span id="'+infos.id+'_message" class="card-url-preview"><p></p> </span></div>');
                    break;
                    ...
                    case "flow":
                    $cardActions = $cardActionsExtended;
                    $cardAnalytics = $cardAnalyticsLimitedFlow;
                    $operator_content = $('<div class="flowchart-operator-content">' +
                        '<span id="'+infos.id+'_message" class="card-text-preview"> </span></div>');
                    break;
                ...
                case "phone":
                    $cardActions = $cardActionsLimited;
                    $operator_content = $('<div class="flowchart-operator-content">' +
                        '<span id="'+infos.id+'_message" class="card-phone-preview"><p></p></span></div>');
                    $cardAnalytics = false;
                    break;
            ...
            if ($cardAnalytics) {
                $cardAnalytics.append($cardActions);
                $cardAnalytics.appendTo($operator);
            }
            else
                $cardActions.appendTo($operator);
        ...
        _createSubConnector: function (connectorKey, connectorInfos, fullElement) {
            ...
            if (typeof connectorInfos.class === "undefined")
                connectorInfos.class = "";
            else
                connectorInfos.class = "-"+connectorInfos.class;
          ...
          /*  if (connectorInfos.type === "button" || connectorInfos.type==="quickreply") {
                $operator_connector_label_clickstats = $('<div class="flowchart-connector-clickstats">' + 0 + ' click(s)</div>');
                $operator_connector_label_clickstats.appendTo($operator_connector);
            }*/
            let color = Flowcomposer.splitTestColors()[connectorInfos.index];

            if (typeof connectorInfos.type !== "undefined" && connectorInfos.type === "variant")
            var $operator_connector_label = $('<div id="'+connectorInfos.id+'_connector" style="color: '+color+';border: 1px solid '+color+';" class="flowchart-operator-connector-label"></div>');
            else
            var $operator_connector_label = $('<div id="'+connectorInfos.id+'_connector" style="" class="flowchart-operator-connector-label'+connectorInfos.class+'"></div>');
            ...
            if (connectorInfos.class=== "-icebreaker")
                var $operator_connector_arrow = $('<div></div>');

            else if (connectorInfos.type === "variant"){
                let borderStyle = "border-left: 10px solid "+color+" !important;";
                var $operator_connector_arrow = $('<div style="'+borderStyle+'" class="flowchart-operator-connector-arrow"></div>');

            }
                else
            var $operator_connector_arrow = $('<div class="flowchart-operator-connector-arrow'+connectorInfos.class+'"></div>');

            $operator_connector_arrow.appendTo($operator_connector);

          // var $operator_connector_small_arrow = $('<div class="flowchart-operator-connector-small-arrow'+connectorInfos.class+'"></div>');
        ...
        _connectorClicked: function (operator, connector, subConnector, connectorCategory) {
            if (connectorCategory == 'outputs') {
                ...
                let that = this;
                setTimeout(function(){
                    that.options.expectsInput = true;
        ...
        colorizeLink: function (linkId, color) {
            ...
            linkData.internal.els.toConnector.css('border-left-color', color);
        },
